...
const WhatsAppButton: React.FC<WhatsAppButtonProps> = ({
  ...
  const getButtonStyles = () => {
    switch (variant) {
      case 'navbar':
        return 'w-9 h-9 rounded-full bg-green-500 hover:bg-green-600 hover:scale-105 transition-all text-white border border-green-400 shadow-sm flex items-center justify-center shrink-0 cursor-pointer';
      case 'hero':
  ...
  return (
    <motion.button
      ...
      onClick={handleClick}
      className={`${getButtonStyles()} ${className}`}
      title={variant === 'navbar' ? 'Chat on WhatsApp' : undefined}
    >
      {variant === 'navbar' ? (
        <svg
          className={getIconSize()}
